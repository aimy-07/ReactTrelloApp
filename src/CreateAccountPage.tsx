...
class LoginPage extends React.Component<IProps, IState> {
    ...
    public componentDidMount() {
        firebaseAuth.onAuthStateChanged(user => {
            this.props.dispatch(actions.changeUser(user));
            // actions.addUser(user);
        });
    }

    public emailLogin(email: string, password: string) {
        this.setFormColor('email');
        if (email === '') {
            this.setState({
        ...
        firebaseAuth.createUserWithEmailAndPassword(email, password)
            .then(user => {
                console.log('Succeeded SignUp with email');
                console.log(user);
                this.setState({
                    email: '',
                    password: '',
            ...
            .catch(error => {
                console.log('Failed SignUp with email');
                this.setState({
                    errorState: ERROR_STATE.EMAIL_SAME,
                });
            });
    }

    // メールアドレスのvalidateチェック関数
    ...
    public setFormColor(inputType: string) {
        if (inputType === 'email') {
            if (this.state.errorState === ERROR_STATE.EMAIL_ERROR_ENPTY || this.state.errorState === ERROR_STATE.EMAIL_ERROR_WRONG || this.state.errorState === ERROR_STATE.EMAIL_SAME) {
                // 赤
                return {
